...
void BookmarksManager::add( const QString & bookmark, bool update )
{
    ...
    logInfo() << "Adding bookmark \"" << bookmark << "\"" << Qt::endl;
...
void BookmarksManager::remove( const QString & bookmark, bool update )
{
    ...
    logInfo() << "Removing bookmark \"" << bookmark << "\"" << Qt::endl;
...
void BookmarksManager::rebuildBookmarksMenu()
{
    if ( ! _bookmarksMenu )
    {
        logError() << "NULL _bookmarksMenu" << Qt::endl;
        return;
    }
...
{
    _baseUrl = newBaseUrl;
    // logDebug() << "Base URL: \"" << _baseUrl << "\"" << Qt::endl;
...
void BookmarksManager::read()
{
    ...
    {
    logError() << "Can't open " << bookmarksFileName() << Qt::endl;
	return;
    }
    ...
    sort();
    logInfo() << _bookmarks.size() << " bookmarks read from " << BookmarksFile << Qt::endl;

#if 0
    foreach ( const QString & bookmark, _bookmarks )
        logDebug() << "Read bookmark \"" << bookmark << "\"" << Qt::endl;
#endif
}
...
void BookmarksManager::write()
{
    if ( ! _dirty )
    {
        logDebug() << "No changes to write to " << BookmarksFile << Qt::endl;
        return;
    }
    ...
    logInfo() << _bookmarks.size() << " bookmarks written to " << BookmarksFile << Qt::endl;
    _dirty = false;
}
